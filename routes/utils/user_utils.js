...
async function chageMealOrderAbove(user_id, recipe_id, recipe_count){
    ...
    await DButils.execQuery(`update userMeals set recipe_count = '${recipe_count - 1}' where user_id='${user_id}' and recipe_id ='${recipe_id}'`);
    let otherId1 = await DButils.execQuery(`select recipe_id from userMeals where user_id='${user_id}' and recipe_count = '${recipe_count - 1}'`);
    await DButils.execQuery(`update userMeals set recipe_count = '${recipe_count}' where user_id='${user_id}' and recipe_id ='${otherId}'`);
    let otherId2 = await DButils.execQuery(`select recipe_id from userMeals where user_id='${user_id}' and recipe_count = '${recipe_count - 1}'`);
...
async function chageMealOrderBellow(user_id,recipe_id ,recipe_count){
    ...
    let otherId2 = await DButils.execQuery(`select recipe_id from userMeals where user_id='${user_id}' and recipe_count = '${recipe_count + 1}'`);
    await DButils.execQuery(`update userMeals set recipe_count = '${recipe_count}' where user_id='${user_id}' and recipe_id ='${otherId}'`);    
    let otherId3 = await DButils.execQuery(`select recipe_id from userMeals where user_id='${user_id}' and recipe_count = '${recipe_count + 1}'`);
...
async function RemoveRecipeFromMeal(user_id, recipe_id){
    ...
    const count2 = await DButils.execQuery(`select recipe_count from userMeals where user_id='${user_id}' and recipe_id='${recipe_id}' `);
    await DButils.execQuery(`update userMeals set recipe_count = recipe_count - 1 where recipe_count > '${Object.values(count[0])[0]}' `);        
    const count3 = await DButils.execQuery(`select recipe_count from userMeals where user_id='${user_id}' and recipe_id='${recipe_id}' `);
}
